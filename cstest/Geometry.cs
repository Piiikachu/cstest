...
namespace cstest
{
    ...
    {
        public const double EPSSQ = 1.0e-16;
        public const double EPSSQNEG = -1.0e-16;
        public const double EPSSELF = 1.0e-6;

        enum Enum1{ OUTSIDE, INSIDE, ONSURF2OUT, ONSURF2IN };
        ...
        public int line_quad_intersect(double[] v0, double[] v1, double[] norm,
            double[] lo, double[] hi)
        {
            int sum, side;
            double xlo, xhi, ylo, yhi, param;
            double[] b=new double[3], e=new double[3], point=new double[3];
            ...
            if (line_line_intersect(b, e, v0, v1, norm, point, out param, out side)) return 1;
                          ...
                          double[] lo, double[] hi, double[] xc)
        {
            int side;
            double xlo, xhi, ylo, yhi, param;
            double[] b=new double[3], e=new double[3];
            ...
            b[0] = xlo; b[1] = ylo; b[2] = 0.0;
            e[0] = xhi; e[1] = ylo; e[2] = 0.0;
            if (line_line_intersect(b, e, v0, v1, norm, xc, out param, out side)) return 1;
                     ...
                     double[] lo, double[] hi)
        {
            ...
            int dim = iface / 2;
            int other = (dim!=0) ? 0 : 1;
            double value = (iface % 2!=0) ? hi[dim] : lo[dim];
                      ...
                      double[] lo, double[] hi)
        {
            ...
            double[] b=new double[3], e = new double[3], h0 = new double[3], 
                h1 = new double[3], h2 = new double[3], h3 = new double[3], 
                n = new double[3], point = new double[3];
            ...
            if (line_tri_intersect(b, e, v0, v1, v2, norm, point, out param, out side)) return 1;
            ...
            if (line_tri_intersect(v0, v1, h0, h1, h2, n, point, out param, out side) ||
                line_tri_intersect(v1, v2, h0, h1, h2, n, point, out param, out side) ||
                line_tri_intersect(v2, v0, h0, h1, h2, n, point, out param, out side) ||
                line_tri_intersect(v0, v1, h0, h2, h3, n, point, out param, out side) ||
                line_tri_intersect(v1, v2, h0, h2, h3, n, point, out param, out side) ||
                line_tri_intersect(v2, v0, h0, h2, h3, n, point, out param, out side)) return 1;
                        ...
                        double[] lo, double[] hi, double[] xc)
        {
            ...
            double xlo, xhi, ylo, yhi, zlo, zhi, param;
            double[] b = new double[3], e = new double[3], h0 = new double[3],
                h1 = new double[3], h2 = new double[3], h3 = new double[3], n = new double[3];
            ...
            b[0] = xlo; b[1] = ylo; b[2] = zlo;
            e[0] = xhi; e[1] = ylo; e[2] = zlo;
            if (line_tri_intersect(b, e, v0, v1, v2, norm, xc, out param, out side)) return 1;
            ...
            if (line_tri_intersect(v0, v1, h0, h1, h2, n, xc, out param, out side) ||
                line_tri_intersect(v1, v2, h0, h1, h2, n, xc, out param, out side) ||
                line_tri_intersect(v2, v0, h0, h1, h2, n, xc, out param, out side) ||
                line_tri_intersect(v0, v1, h0, h2, h3, n, xc, out param, out side) ||
                line_tri_intersect(v1, v2, h0, h2, h3, n, xc, out param, out side) ||
                line_tri_intersect(v2, v0, h0, h2, h3, n, xc, out param, out side)) return 1;
        ...
        public static bool line_line_intersect(double[] start, double[] stop,
                     double[] v0, double[] v1, double[] norm,
                     double[] point,out double param,out int side)
        {
            double[] vec = new double[3], start2stop = new double[3], 
                edge = new double[3], pvec = new double[3];
            ...
            if (dotstart < 0.0) side = (int)Enum1.INSIDE;
            else if (dotstart > 0.0) side = (int)Enum1.OUTSIDE;
            else if (dotstop > 0.0) side = (int)Enum1.ONSURF2OUT;
            else side = (int)Enum1.ONSURF2IN;
        ...
        public static bool axi_line_intersect(double tdelta, double[] x, double[] v,
                                int outface, double[] lo, double[] hi,
                                double[] v1, double[] v2, double[] norm, int selfflag,
                                double[] xc, double[] vc,out double param,out int side)
        {
            ...
            if (v1[0] == v2[0])
            {
                ...
                else
                {
                    ...
                    t1 = (v1[0] - x[0]) / v[0];
                    if (selfflag!=0 && t1 == 0.0) return false;
                }
            ...
            else if (v1[1] == v2[1])
            {
                ...
                {
                    if (!axi_horizontal_line(tdelta, x, v, v1[1],out nc,out t1,out t2)) return false;
                }
            ...
            else
            {
                ...
                double sarg = Math.Sqrt(arg);

                nc = 2;
                double tone = (-b - sarg) / a;
                double ttwo = (-b + sarg) / a;
                t1 = Math.Min(tone, ttwo);
                t2 = Math.Max(tone, ttwo);
            }
            ...
            if (selfflag!=0 && nc == 2)
            {
                if (Math.Abs(t1) < Math.Abs(t2)) t1 = t2;
                nc = 1;
            }
            ...
            while (true)
            {
                ...
                */
                ...
                xc[1] = Math.Sqrt(ynew * ynew + znew * znew);
            ...
            */
            ...
            if (dot < 0.0) side = (int)Enum1.OUTSIDE;
            else side = (int)Enum1.INSIDE;
        ...
        public static bool axi_horizontal_line(double tdelta, double[] x, double[] v,
                                 double yhoriz,out int nc,out double t1,out double t2)
        {
            t1 = t2 =nc= 0;
        ...
        public static bool line_tri_intersect(double[] start, double[] stop,
                    double[] v0, double[] v1, double[] v2, double[] norm,
                    double[] point,out double param,out int side)
        {
            double[] vec=new double[3], start2stop = new double[3], 
                edge = new double[3], pvec = new double[3], xproduct = new double[3];
        ...
        double distsq_point_tri(double[] x, double[] p1, double[] p2, double[] p3,
                                double[] norm)
        {
            double[] a = new double[3], point = new double[3], edge = new double[3],
                pvec = new double[3], xproduct = new double[3];
            ...
            rsq = Math.Min(rsq, distsq_point_line(point, p2, p3));
            rsq = Math.Min(rsq, distsq_point_line(point, p3, p1));
            return rsq + pdistsq;
        }
        ...
        {
            double distsq;
            double[] pt = new double[3], e1 = new double[3], e2 = new double[3];
            ...
            pt[0] = hi[0]; pt[1] = lo[1];
            distsq = Math.Min(distsq, distsq_point_line(pt, x, y));
            pt[0] = lo[0]; pt[1] = hi[1];
            distsq = Math.Min(distsq, distsq_point_line(pt, x, y));
            pt[0] = hi[0]; pt[1] = hi[1];
            distsq = Math.Min(distsq, distsq_point_line(pt, x, y));
            ...
            distsq = Math.Min(distsq, distsq_point_line(x, e1, e2));
            distsq = Math.Min(distsq, distsq_point_line(y, e1, e2));
                            ...
                            double[] lo, double[] hi)
        {
            ...
            distsq = Math.Min(distsq, distsq_point_tri(pt[1], x, y, z, norm));
            distsq = Math.Min(distsq, distsq_point_tri(pt[2], x, y, z, norm));
            distsq = Math.Min(distsq, distsq_point_tri(pt[3], x, y, z, norm));
            distsq = Math.Min(distsq, distsq_point_tri(pt[4], x, y, z, norm));
            distsq = Math.Min(distsq, distsq_point_tri(pt[5], x, y, z, norm));
            distsq = Math.Min(distsq, distsq_point_tri(pt[6], x, y, z, norm));
            distsq = Math.Min(distsq, distsq_point_tri(pt[7], x, y, z, norm));

            // distance between tri corner pts and 6 hex faces (2 tris per face)

            face[0] = -1.0; face[1] = 0.0; face[2] = 0.0;
            distsq = Math.Min(distsq, distsq_point_tri(x, pt[0], pt[4], pt[6], face));
            distsq = Math.Min(distsq, distsq_point_tri(y, pt[0], pt[4], pt[6], face));
            distsq = Math.Min(distsq, distsq_point_tri(z, pt[0], pt[4], pt[6], face));
            distsq = Math.Min(distsq, distsq_point_tri(x, pt[0], pt[6], pt[2], face));
            distsq = Math.Min(distsq, distsq_point_tri(y, pt[0], pt[6], pt[2], face));
            distsq = Math.Min(distsq, distsq_point_tri(z, pt[0], pt[6], pt[2], face));

            face[0] = 1.0; face[1] = 0.0; face[2] = 0.0;
            distsq = Math.Min(distsq, distsq_point_tri(x, pt[1], pt[3], pt[7], face));
            distsq = Math.Min(distsq, distsq_point_tri(y, pt[1], pt[3], pt[7], face));
            distsq = Math.Min(distsq, distsq_point_tri(z, pt[1], pt[3], pt[7], face));
            distsq = Math.Min(distsq, distsq_point_tri(x, pt[1], pt[7], pt[5], face));
            distsq = Math.Min(distsq, distsq_point_tri(y, pt[1], pt[7], pt[5], face));
            distsq = Math.Min(distsq, distsq_point_tri(z, pt[1], pt[7], pt[5], face));

            face[0] = 0.0; face[1] = -1.0; face[2] = 0.0;
            distsq = Math.Min(distsq, distsq_point_tri(x, pt[0], pt[1], pt[5], face));
            distsq = Math.Min(distsq, distsq_point_tri(y, pt[0], pt[1], pt[5], face));
            distsq = Math.Min(distsq, distsq_point_tri(z, pt[0], pt[1], pt[5], face));
            distsq = Math.Min(distsq, distsq_point_tri(x, pt[0], pt[5], pt[4], face));
            distsq = Math.Min(distsq, distsq_point_tri(y, pt[0], pt[5], pt[4], face));
            distsq = Math.Min(distsq, distsq_point_tri(z, pt[0], pt[5], pt[4], face));

            face[0] = 0.0; face[1] = 1.0; face[2] = 0.0;
            distsq = Math.Min(distsq, distsq_point_tri(x, pt[2], pt[6], pt[7], face));
            distsq = Math.Min(distsq, distsq_point_tri(y, pt[2], pt[6], pt[7], face));
            distsq = Math.Min(distsq, distsq_point_tri(z, pt[2], pt[6], pt[7], face));
            distsq = Math.Min(distsq, distsq_point_tri(x, pt[2], pt[7], pt[3], face));
            distsq = Math.Min(distsq, distsq_point_tri(y, pt[2], pt[7], pt[3], face));
            distsq = Math.Min(distsq, distsq_point_tri(z, pt[2], pt[7], pt[3], face));

            face[0] = 0.0; face[1] = 0.0; face[2] = -1.0;
            distsq = Math.Min(distsq, distsq_point_tri(x, pt[0], pt[2], pt[3], face));
            distsq = Math.Min(distsq, distsq_point_tri(y, pt[0], pt[2], pt[3], face));
            distsq = Math.Min(distsq, distsq_point_tri(z, pt[0], pt[2], pt[3], face));
            distsq = Math.Min(distsq, distsq_point_tri(x, pt[0], pt[3], pt[1], face));
            distsq = Math.Min(distsq, distsq_point_tri(y, pt[0], pt[3], pt[1], face));
            distsq = Math.Min(distsq, distsq_point_tri(z, pt[0], pt[3], pt[1], face));

            face[0] = 0.0; face[1] = 0.0; face[2] = 1.0;
            distsq = Math.Min(distsq, distsq_point_tri(x, pt[4], pt[5], pt[7], face));
            distsq = Math.Min(distsq, distsq_point_tri(y, pt[4], pt[5], pt[7], face));
            distsq = Math.Min(distsq, distsq_point_tri(z, pt[4], pt[5], pt[7], face));
            distsq = Math.Min(distsq, distsq_point_tri(x, pt[4], pt[7], pt[6], face));
            distsq = Math.Min(distsq, distsq_point_tri(y, pt[4], pt[7], pt[6], face));
            distsq = Math.Min(distsq, distsq_point_tri(z, pt[4], pt[7], pt[6], face));
    ...
    double line_fraction(double[] x, double[] v0, double[] v1)
    {
        ...
        fracsq = Math.Min(fracsq, MathExtra.lensq3(segment) / lensq);
    ...
    double tri_fraction(double[] x, double[] v0, double[] v1, double[] v2)
    {
        ...
        lensq = Math.Min(lensq, MathExtra.lensq3(segment));
